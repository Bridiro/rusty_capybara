pub mod sensors {
    ...
    impl MPU6050 {
        ...
        pub fn run(&mut self) -> Result<(), rppal::i2c::Error> {
            ...
            self.init().expect("Failed to initialize MPU6050");
            ...
            std::thread::spawn(move || {
                ...
                *running.lock().unwrap() = true;
                while running.lock().unwrap().clone() {
                    ...
                    let acc_angle_x =
                        (acc_y / (acc_x.powi(2) + acc_z.powi(2)).sqrt()).atan() * 180.0 / PI
                            - acc_x_err;
                    let acc_angle_y =
        ...
        fn calculate_error(&mut self) -> Result<(f32, f32, f32, f32, f32, f32), Box<dyn Error>> {
            ...
            for _ in 0..500 {
                acc_x +=
                    read_raw_data(&mut self.i2c.lock().unwrap(), ACCEL_XOUT_H)? as f32 / 16384.0;
                acc_y +=
                    read_raw_data(&mut self.i2c.lock().unwrap(), ACCEL_YOUT_H)? as f32 / 16384.0;
                acc_z +=
                    read_raw_data(&mut self.i2c.lock().unwrap(), ACCEL_ZOUT_H)? as f32 / 16384.0;
